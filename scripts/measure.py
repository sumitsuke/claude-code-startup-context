...
io.open(SETTINGS_MIN_MEMORY, "w", encoding="utf-8").write(json.dumps({"autoMemoryDirectory": MEM_DIR.replace("\\", "/")}))
...
def run(label, extra=()):
    ...
    usage = None; version = None; attachments = {}; mcp_names = set()
    ...
    print(f"{label}: total {total:,}  (in {usage['input_tokens']}  cache_create {usage['cache_creation_input_tokens']:,}  "
          f"cache_read {usage['cache_read_input_tokens']:,})  v{version}  {sec}s  mcp tool names {len(mcp_names)}")
    return {"label": label, "session_id": sid, "total": total, **usage, "claude_code_version": version,
            "seconds": sec, "mcp_tool_names": len(mcp_names), "attachment_chars": attachments}
...
def measured(label, extra=(), expect_mcp=None):
    for attempt in range(1, 4):
        ...
        if expect_mcp is None or r["mcp_tool_names"] == expect_mcp:
            r["valid"] = True
            return r
        print(f"  discarded: MCP tool names {r['mcp_tool_names']} != {expect_mcp} (tool list incomplete when the first turn was sent) - re-running")
        r["valid"] = False; r["discard_reason"] = f"mcp_tool_names {r['mcp_tool_names']} != {expect_mcp}"
        discarded.append(r)
...
doc = {"tool": "claude-code-startup-context/scripts/measure.py", "schema": 1,
       "measured_at": datetime.datetime.now().astimezone().isoformat(timespec="minutes"),
       "claude_code_version": runs[0]["claude_code_version"], "model": MODEL, "prompt": PROMPT,
       "total_input_definition": "input_tokens + cache_creation_input_tokens + cache_read_input_tokens (first assistant turn)",
       "runs": runs, "discarded_runs": discarded, "summary": summary}
...
s = summary
md = [f"# Claude Code startup input — remove-and-restore ({doc['measured_at']}, Claude Code {doc['claude_code_version']}, {MODEL})", "",
      "| run | total input | vs A1 | input / cache_create / cache_read | mcp tool names | s | attempt |",
      "|---|---|---|---|---|---|---|"]
for r in runs:
    md.append(f"| {r['label']} | {r['total']:,} | {r['total'] - A1:+,} | {r['input_tokens']} / {r['cache_creation_input_tokens']:,} / "
              f"{r['cache_read_input_tokens']:,} | {r['mcp_tool_names']} | {r['seconds']} | {r['attempt']} |")
for r in discarded:
    md.append(f"| ~~{r['label']}~~ discarded ({r['discard_reason']}) | {r['total']:,} | {r['total'] - A1:+,} | {r['input_tokens']} / {r['cache_creation_input_tokens']:,} / "
              f"{r['cache_read_input_tokens']:,} | {r['mcp_tool_names']} | {r['seconds']} | {r['attempt']} |")
md += ["",
       ...
       f"- A1 attachment sizes (chars of JSON, from the transcript): " + ", ".join(f"{k} {v:,}" for k, v in runs[0]["attachment_chars"].items()),
